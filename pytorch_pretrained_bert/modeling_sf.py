...
# CONFIG_NAME = 'bert_config_less_layer.json'
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, task_idx=None):
        ...
        # embeddings = words_embeddings + token_type_embeddings  # 去掉position_embeddings后效果下降明显，会出现字/词重复的情况。
...
class CNN_Text(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args.class_num = 4
        self.args.cnn_kernel_num = 100
        self.args.cnn_kernel_sizes = [3, 4, 5]
        
        V = self.args.vocab_size
        D = self.args.hidden_size
        C = self.args.class_num
        Ci = 1
        Co = self.args.cnn_kernel_num
        Ks = self.args.cnn_kernel_sizes
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def cnn_trans_input_id2emb(self, input_ids, input_length):
        """
        ...
        input_ids = torch.index_select(input_ids, 1, torch.tensor(range(1, input_length-1)).cuda())
        # cnnText = CNN_Text(self.config)
        # cnnText = cnnText.cuda()
        cnn_pred_labels = []
        if input_ids is not None:
            for i, stu_com_id in enumerate(input_ids):
                ...
                gumbel_logit = self.cnnText.gumbel_softmax(logit)
                one_cnn_result = self.cnnText.comment_choice(word_com_ids.float(), gumbel_logit)  # 实现点评项的重排序
                one_cnn_result = one_cnn_result.view(1, -1)
                one_cnn_result = torch.cat([one_cnn_result, torch.zeros((1, stu_com_id.size(0)-one_cnn_result.size(1))).cuda()], dim=1)
                if i == 0:
                    cnn_result = one_cnn_result
                else:
                    cnn_result = torch.cat([cnn_result, one_cnn_result], dim=0)
            input_embs = torch.cat([cls_id, cnn_result.long(), sep_id], dim=1)
        return input_embs, cnn_pred_labels
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        # trans_input_id, cnn_pred_label = self.cnn_trans_input_id2emb(input_ids, input_length)
