...
from pytorch_pretrained_bert.cnn_classifier import CNN_Model, RCNN_Model
...
class BertEncoder(nn.Module):  # bert的encode部分，即多层的bert模块 add by sfeng
    ...
    def forward(self, hidden_states, attention_mask, analogy_attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, seg_ids=None):
        ...
        # if analogy_attention_mask is None:
        #     analogy_attention_mask = attention_mask
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                if i < int(self.num_hidden_layers/2):  # add by sfeng; (split for two part, first for question inner attention, second for question outer attention)
...
class PreTrainedBertModel(nn.Module):  # 根据输入参数的维度，对bert原本的参数做调整，使其适配新的输入（维度方面）； add by sfeng
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        # 如果要改bert词表数据，需要在这里添加对词表的筛选或者初始值添加（如果添加初始值的话这些初始值没有初始化的预训练参数，为随机参数，效果如何不可知）； by sfeng;
            ...
            if len(state_dict[_k].data) < config.vocab_size:
                for _vocab in _source_vocab:
                    ...
                    if isnumber(_vocab):
                        ...
                        # init_vocab_emb = torch.Tensor(1, len(state_dict[_k].data[0]))
                        # init_vocab_emb = torch.zeros(1, len(state_dict[_k].data[0]))
                        rand_num = random.sample([i for i in range(len(state_dict[_k].data))], 5)
                        for rand_n in rand_num:  # 逐字符词向量平均得到近似初始词向量
...
# 更新后的copynet的另一种实现方式（按copynet原论文的来）
...
class RetrieveToGenerate(nn.Module):

    def __init__(self, config, args, topk=2):
        super(RetrieveToGenerate, self).__init__()
        self.topk = topk
        self.args = args

    def choice_topk(self, logits, query_simids):
        """从top30中取出topk个相似题
        query_simids: [[query_id, sim_id,..], [..]]
        """  
        topn = self.args.retrieve_topn + 1
        new_query_simids = []
        # logits = self.retModel(input_ids)
        for i in range(int(len(logits)/topn)):
            query_logit = logits[i*topn]
            sim_logit = logits[i*topn+1:(i+1)*topn]
            sim = torch.matmul(query_logit, sim_logit.t())
            sort_logit, indices = torch.sort(sim, descending=True)
            if i == 0:
                query_sim_logits = sort_logit[:self.topk].unsqueeze(0)
            else:
                query_sim_logits = torch.cat([query_sim_logits, sort_logit[:self.topk].unsqueeze(0)], 0)
            new_query_simids.append([query_simids[i*topn]] + [query_simids[i*topn+1+idx] for idx in indices[:self.topk]])  # +1是为了跳过query_id
        return query_sim_logits, new_query_simids

    def retrieve_question(self, bi_uni_pipeline, id_questions, new_query_simids):
        """根据上一步得到的topk相似题，将query question与其依次拼接作为一个样本
        """
        instances = []
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        for sample in new_query_simids:
            query_question = id_questions[sample[0]]
            retrieve_questions = [id_questions[sim_id] for sim_id in sample[1:]]
            for top in range(self.topk):
                pro_instance = bi_uni_pipeline(query_question + retrieve_questions[top])
                instances.append(pro_instance)
        batch, oov_tokens = batch_list_to_batch_tensors(instances, is_train=False)  
        batch = [t.to(device) if t is not None else None for t in batch]
        return batch, oov_tokens

    def get_retrieve_question(self, id_instances, new_query_simids):
        """根据上一步得到的topk相似题，提取id_instances(query_question与retriver_question提前拼接好，通过索引获取)中的instance部分
        """
        batch = []
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        for sample in new_query_simids:
            ...
            simids = sample[1:]
            for top in range(self.topk):
                batch.append(id_instances[(id, simids[top])])
        batch = [t.to(device) if t is not None else None for t in batch]
        return batch
    ...
    def marginalize(self, seq_logits, doc_scores):
        ...
        seq_logprobs = torch.log(seq_logits+1e-8)
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False, add_memory_module=False, max_len_a=192,
            max_len_b=64, add_copynet=False, add_analogy_embedding=False, vocabs=[], topk=2, args=None):
        super(BertForPreTrainingLossMask, self).__init__(config)
        # self.bert = BertModel(config)
        self.topk = topk
        self.vocabs = vocabs
        self.args = args
        self.add_analogy_embedding = add_analogy_embedding
        self.Ret2Gen = RetrieveToGenerate(config, args, topk)
        if self.args.retrieve_model_name == 'cnn':
            self.RetModel = CNN_Model(config)
        elif self.args.retrieve_model_name == 'rcnn':
            self.RetModel = RCNN_Model(config)
        elif self.args.retrieve_model_name == 'transformer':
            config.num_hidden_layers = 2
            self.RetModel = BertModelIncr(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                masked_pos=None, masked_weights=None, task_idx=None,  masked_pos_2=None, masked_weights_2=None, 
                masked_labels_2=None, mask_qkv=None, analogy_attention_mask=None, step=0, position_ids=None, 
                num_equ_ids=None, extend_oov_query_qsids=None, extra_zeros=None, question_sim_ids=None, bi_uni_pipeline=[], 
                id_questions=None, id_instances=None):
        ...
        # 构造检索器，并根据检索器结果得到topk, 同时获取topk的相似度概率
        if self.args.retrieve_model_name in ['cnn', 'rcnn']:
            ret_logits = self.RetModel(input_ids)
        elif self.args.retrieve_model_name == 'transformer':
            _, _, ret_logits, _ = self.RetModel(input_ids, token_type_ids, position_ids, attention_mask, analogy_attention_mask, 
                        output_all_encoded_layers=True, mask_qkv=mask_qkv, task_idx=task_idx, num_equ_ids=num_equ_ids)
        else:
            # 与生成器的bert参数共享
            _, _, ret_logits, _ = self.bert(input_ids, token_type_ids, position_ids, attention_mask, analogy_attention_mask, 
                        output_all_encoded_layers=True, mask_qkv=mask_qkv, task_idx=task_idx, num_equ_ids=num_equ_ids)

        query_sim_logits, new_question_simids = self.Ret2Gen.choice_topk(ret_logits, question_sim_ids)
        batch, oov_tokens = self.Ret2Gen.retrieve_question(bi_uni_pipeline, id_questions, new_question_simids)

        input_ids, token_type_ids, attention_mask, mask_qkv, masked_labels_2, masked_pos_2, masked_weights_2, \
                    masked_lm_labels, masked_pos, masked_weights, is_next, task_idx, analogy_attention_mask, position_ids, \
        ...
        embedding_output, sequence_output, pooled_output, attn_dists = self.bert(input_ids, token_type_ids, position_ids, attention_mask, analogy_attention_mask, 
                        output_all_encoded_layers=True, mask_qkv=mask_qkv, task_idx=task_idx, num_equ_ids=num_equ_ids)
        sequence_output_6layer = sequence_output[5]
        sequence_output = sequence_output[-1]
        attn_dist_6layer = attn_dists[5]
        ...
        if self.add_copynet:
            ...
            gen_logits = self.copyNet(x_e, dec_input, attn_dist, extend_oov_query_qsids, extra_zeros)

            log_scores = self.Ret2Gen.marginalize(gen_logits, query_sim_logits) 
        ...
        if self.add_copynet:
            if step % 5000 == 0:
                logging.warn('masked_lm_loss: {:.4f}, copy_loss: {:.4f}'.format(masked_lm_loss, copy_loss))
            if step > 0 and step % 10000 == 0:
                log_max_value, log_max_ids = torch.max(attn_dists[-1], dim=-1)  # 取出概率最大的id
                logger.info('attn_dists[-1]:{}'.format([[self.vocabs[input_ids[0][gen_id]], round(log_max_value[0].detach().cpu().numpy()[i+448], 2)] for i, gen_id in enumerate(log_max_ids[0][448:].detach().cpu().numpy())][:15]))
                logger.info('target:{}'.format([self.vocabs[input_ids[0][gen_id+448]] for gen_id in range(len(input_ids[0][448:]))][:15]))
            ...
            if self.cls2 is not None and masked_pos_2 is not None:
                ...
                if step % 2000 == 0:
                    logging.warn('masked_lm_loss: {:.4f}, masked_lm_loss_2: {:.4f}'.format(masked_lm_loss, masked_lm_loss_2))
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0, search_beam_size=1, length_penalty=1.0, 
                ...
                max_len_b=64, add_copynet=False, add_analogy_embedding=False, vocabs=[], args=None):
        super(BertForSeq2SeqDecoder, self).__init__(config)
        self.vocabs = vocabs
        self.args = args
        self.add_analogy_embedding = add_analogy_embedding
        self.bert = BertModelIncr(config)
        self.Ret2Gen = RetrieveToGenerate(config, self.args, topk)
        if args.retrieve_model_name == 'cnn':
            self.RetModel = CNN_Model(config)
        elif self.args.retrieve_model_name == 'rcnn':
            self.RetModel = RCNN_Model(config)
        elif self.args.retrieve_model_name == 'transformer':
            config.num_hidden_layers = 2
            # self.Transformer_embeddings = BertEmbeddings(config)
            self.RetModel = BertModelIncr(config)
            config.num_hidden_layers = 12
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None, 
            ...
            question_sim_ids=None, bi_uni_pipeline=None, id_questions=None):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx, mask_qkv=mask_qkv, 
                ...
                question_sim_ids=question_sim_ids, bi_uni_pipeline=bi_uni_pipeline, id_questions=id_questions)
        ...
        query_sim_logits, new_question_simids = self.Ret2Gen.choice_topk(ret_logits, question_sim_ids)
        batch, oov_tokens = self.Ret2Gen.retrieve_question(bi_uni_pipeline, id_questions, new_question_simids)
        input_ids, token_type_ids, attention_mask, mask_qkv, task_idx, analogy_attention_mask, position_ids, \
                    num_equ_ids, extend_oov_query_qsids, extra_zeros = batch
        # 记录检索器粗排及精排的相似题目id.
        qs_simids = {question_sim_ids[i]: question_sim_ids[i+1:i+self.args.retrieve_topn+1] for i in range(0, len(question_sim_ids), self.args.retrieve_topn+1)}
        new_qs_simids = {new_qs_simid[0]: new_qs_simid[1:] for new_qs_simid in new_question_simids}
        ...
        output_ids = []
        log_scores_ids, gen_score_ids, ze_score_ids, xq_score_ids = [], [], [], []
        prev_embedding = None
        ...
        while next_pos < output_length:
            ...
            sequence_output_6layer = new_encoded_layers[5]
            attn_dist_6layer = attn_dists[5]
            ...
            # log_max_value, log_max_ids = torch.max(log_scores, dim=-1)  # 取出概率最大的id
            # logger.info('pred log_scores:{}'.format([self.vocabs[gen_id[0]] for gen_id in np.array(log_max_ids.cpu())]))
            ...
            # max_ids = torch.tensor([[genCopy_output_vocabIds[id[0]]] for id in max_ids]).cuda()
        ...
        return torch.cat(output_ids, dim=1), oov_tokens, retrieve_simids


    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None,
            analogy_attention_mask=None, retrieve_batch=None, num_equ_ids=None, extend_oov_query_qsids=[], extra_zeros=[],
            question_sim_ids=None, bi_uni_pipeline=[], id_questions=None):
    # def beam_search(self, input_ids, question_sim_ids=None, bi_uni_pipeline=[], id_questions=None):
        ...
        while next_pos < output_length:
            ...
            if self.add_copynet:
                ...
                if start_pos == 0:
                    ...
                    # attn_dist = attn_dist / normalization_factor
                    attn_dist = attn_dist / (normalization_factor + 1e-8)
                # probs = self.copyNet(x_e, dec_input, attn_dist, extend_oov_query_qsids, extra_zeros)
                # log_scores = torch.log(probs+1e-8)
                # add for end2end.
                gen_logits = self.copyNet(x_e, dec_input, attn_dist, extend_oov_query_qsids, extra_zeros)
                log_scores = self.Ret2Gen.marginalize(gen_logits, query_sim_logits) 
            ...
            if len(total_scores) == 0:
                ...
                back_ptrs = torch.div(k_ids, K)  # back_ptrs变量作用？(答：取出上一步beamsize范围内的值，4.13 by sfeng;)
            ...
            if self.pos_shift:
                if prev_embedding is None:
                    ...
                    prev_embedding = torch.cat(
                        (prev_embedding, new_embedding), dim=1)
                    if self.add_memory_module:
                        tmp_back_ptrs = back_ptrs.unsqueeze(1).expand(-1,self.topk,-1).contiguous().view(-1,back_ptrs.size(1)) ## TODO: 修改back_ptrs维度
            ...
            if is_first:
                ...
                query_sim_logits = first_expand(query_sim_logits)
        ...
        return traces, oov_tokens, retrieve_simids
