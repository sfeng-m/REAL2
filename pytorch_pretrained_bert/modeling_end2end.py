...
# from .cnn_classifier import CNN_Text, comment_choice
from transformers import BertForTokenClassification as NEWBertForTokenClassification
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, task_idx=None):
        ...
        zero_ids = torch.zeros(input_ids.size(0), input_ids.size(1))
        if position_ids is None:
            position_ids = torch.arange(
                seq_length, dtype=torch.long, device=input_ids.device)
            # position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
            position_ids = position_ids.unsqueeze(0).expand_as(zero_ids)
        if token_type_ids is None:
            # token_type_ids = torch.zeros_like(input_ids)
            token_type_ids = torch.zeros_like(zero_ids)

        if input_ids.size(-1) == self.config.hidden_size:
...
class BertPooler(nn.Module):  # 第一个token的接全连接层，得到输出结果（eg:分类任务取该结果）
    def __init__(self, config):
...
class PreTrainedBertModel(nn.Module):  # 根据输入参数的维度(即目标输出维度)，对bert原本的参数做调整，使其适配新的输入（维度方面）； add by sfeng
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        """
        tempdir = None
        if os.path.isdir('./pytorch_pretrained_bert/bert_parameter'):
            serialization_dir = './pytorch_pretrained_bert/bert_parameter'
        else:
        ...
        if state_dict is None:
            ...
            if is_roberta:
                weights_path = os.path.join('../comment/tmp_folder', WEIGHTS_NAME)
            else:
                weights_path = os.path.join(serialization_dir, WEIGHTS_NAME)
        ...
        # state_dict['loss_beta'] = nn.Parameter(torch.randn(1))  # loss=lm_loss+rl_loss,故增加一个可训练系数； add by sfeng
        # state_dict['loss_beta'] = nn.Parameter(torch.FloatTensor([1]), requires_grad=True)  # loss=lm_loss+rl_loss,故增加一个可训练系数； add by sfeng

        # state_dict['loss_beta'] = model.loss_beta  # loss=lm_loss+rl_loss,故增加一个可训练系数； add by sfeng(使用这个, 2020/4/9)
        # print('model.loss_beta:', model.loss_beta)
...
class classifier_to_generation():
    # def __init__(self):

    def gumbel_softmax(self, logit):
        # gumbel softmax for discretized data
        return F.gumbel_softmax(logit, tau=1, hard=True)

    def kronecker(self, A, B):
        AB = torch.einsum("ab,cd->acbd", A, B)
        AB = AB.view(A.size(0)*B.size(0), A.size(1)*B.size(1))
        AB = AB.view(A.size(1), B.size(1))  # A/B为两个一维向量，转换成A*B的形式
        return AB

    def comment_choice(self, x, logit):
        # 下面几行可实现点评项的重排序；
        for i, (src, tag) in enumerate(zip(x, logit)):
            _src = src.view(1, -1)
            tag = tag.unsqueeze(0)
            if i == 0:
                src_tag = self.kronecker(tag, _src).unsqueeze(1)
            else:
                src_tag = torch.cat([src_tag, self.kronecker(tag, _src).unsqueeze(1)], dim=1)
        data = src_tag.view(-1, src_tag.size(2))
        zeros = torch.zeros(data.size()).cuda()
        mask = data.eq(zeros).all(dim=1).cuda()
        mask = ~mask.unsqueeze(1).expand(data.size())
        result = torch.masked_select(data, mask)
        result = result.view(-1, x.size(-1))
        # print(result)
        return result
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        ...
        # self.cnnText = CNN_Text(config)
        self.bert_num_labels = 3
        self.bert_classifier = NEWBertForTokenClassification.from_pretrained('bert-base-chinese', 
            num_labels = self.bert_num_labels, return_dict=True)
        self.cls2gen = classifier_to_generation()
        ...
        self.crit_mask_lm = nn.CrossEntropyLoss()
        # self.crit_next_sent = nn.CrossEntropyLoss(reduction='none')
        self.crit_bert_classifier = nn.CrossEntropyLoss()
        ...
        self.loss_beta = nn.Parameter(torch.FloatTensor([1]), requires_grad=True)
        self.register_parameter('loss_beta', self.loss_beta)
        self.coff = torch.FloatTensor([1])
        self.smooth = torch.FloatTensor([0.5])
        self.dropout = nn.Dropout(config.hidden_dropout_prob)  # 用于分类器
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                num_tokens_a=None, num_tokens_b=None, mask_qkv=None,  # 此处之后为自己定义的变量;
                no_cuda=True, rl=True, epoch=1, step=0, multi_task=False, input_ids_no_masked=None,
                input_mask_no_masked=None, stu_com_ids=None, comment_ids=None, args=None, 
                perf_label=None, perf_label_mask=None, s2s_sep_idx=None, classifier_attention_mask=None,
                com_cls_idx=None):

        # def gumbel_softmax(logit):
        #     # gumbel softmax for discretized data
        #     return F.gumbel_softmax(logit, tau=1, hard=True)
        ...
        # def comment_choice(x, logit):
        #     # 下面几行可实现点评项的重排序；
        #     for i, (src, tag) in enumerate(zip(x, logit)):
        #         _src = src.view(1, -1)
        #         tag = tag.unsqueeze(0)
        #         if i == 0:
        #             src_tag = kronecker(tag, _src).unsqueeze(1)
        #         else:
        #             src_tag = torch.cat([src_tag, kronecker(tag, _src).unsqueeze(1)], dim=1)
        #     data = src_tag.view(-1, src_tag.size(2))
        #     zeros = torch.zeros(data.size()).cuda()
        #     mask = data.eq(zeros).all(dim=1).cuda()
        #     mask = ~mask.unsqueeze(1).expand(data.size())
        ...
        def bert_mask_classifier_loss(logit, input_label):
            active_loss = input_label.view(-1) != 3
            active_logit = logit.view(-1, self.bert_num_labels)[active_loss]
            active_label = input_label.view(-1)[active_loss]
            if active_label.numel() > 0:
                single_bert_loss = self.crit_bert_classifier(active_logit, active_label)
            else:
                single_bert_loss = torch.Tensor([0]).cuda()[0]
            return single_bert_loss
        ...
        if multi_task:
            ...
            _, pooled_output = self.bert(
                input_ids_no_masked, token_type_ids, input_mask_no_masked, output_all_encoded_layers=False, mask_qkv=mask_qkv,
                task_idx=task_idx)
        else:
            
            if stu_com_ids is not None:
                # classifier_inputs = stu_com_ids.view(-1, 10)
                outputs = self.bert_classifier(input_ids=stu_com_ids, attention_mask=classifier_attention_mask)
                token_logits = outputs.logits
                for i, (input_id, logit, input_label, sep_idx, cls_idx) in enumerate(zip(input_ids, token_logits, perf_label, s2s_sep_idx, com_cls_idx)):
                    # TODO: 取出第一个数（CLS），并在stu_com_id中删除该数
                    CLS = torch.index_select(input_id, 0, torch.tensor([0]).cuda()).view(1, -1)
                    # TODO: 取出SEP之后的内容（评语+padding），并在stu_com_id中删除这些项
                    COM_SEP_END = torch.index_select(input_id, 0, torch.tensor(range(sep_idx[0], len(input_id))).cuda()).view(1, -1)
                    stu_com_id = torch.index_select(input_id, 0, torch.tensor(range(1, sep_idx[0])).cuda())

                    word_com_ids = stu_com_id.view(-1, 10)
                    cls_idx = torch.index_select(cls_idx, 0, torch.tensor(range(word_com_ids.size(0))).cuda())
                    logit = torch.index_select(logit, 0, cls_idx)
                    input_label = torch.index_select(input_label, 0, torch.tensor(range(word_com_ids.size(0))).cuda())

                    single_bert_loss = bert_mask_classifier_loss(logit, input_label)
                    gumbel_logit = self.cls2gen.gumbel_softmax(logit)
                    one_cnn_result = self.cls2gen.comment_choice(word_com_ids.float(), gumbel_logit)  # 实现点评项的重排序
                    one_cnn_result = one_cnn_result.view(1, -1)
                    # TODO: 合并CLS+one_cnn_result+padding
                    one_cnn_result = torch.cat([CLS, one_cnn_result.long(), COM_SEP_END], dim=1)
                    if i == 0:
                        cnn_result = one_cnn_result
                        # logits = logit
                        masked_bert_loss = single_bert_loss
                    else:
                        cnn_result = torch.cat([cnn_result, one_cnn_result], dim=0)
                        # logits = torch.cat([logits, logit], dim=0)
                        masked_bert_loss += single_bert_loss

                sequence_output, pooled_output = self.bert(
                    cnn_result, token_type_ids, attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx)
            else:
                sequence_output, pooled_output = self.bert(
        ...
        def loss_mask_and_normalize_classify(loss, mask, classify_mask):
            mask = mask.type_as(loss)
            classify_mask = classify_mask.unsqueeze(1).expand(-1, loss.size(1))
            classify_mask = classify_mask.type_as(loss)
            loss = loss * mask * classify_mask
            denominator = torch.sum(mask * classify_mask) + 1e-5
            return (loss / denominator).sum()

        def loss_mask_and_normalize_cnn_classify(loss, perf_label_mask):
            # perf_label_mask = perf_label_mask.unsqueeze(1).expand(-1, loss.size(1))
            perf_label_mask = perf_label_mask.type_as(loss)
            loss = loss * perf_label_mask
            denominator = torch.sum(perf_label_mask) + 1e-5
            return (loss / denominator).sum()
        ...
        # masked lm
        sequence_output_masked = gather_seq_out_by_pos(
            sequence_output, masked_pos)  # 这一步是从sequence_output192维的字中找出对应被masked掉的字；add by sfeng
        prediction_scores_masked, seq_relationship_score = self.cls(
            sequence_output_masked, pooled_output, task_idx=task_idx)  # 做一层线性转换； add by sfeng
        ...
        # whether exist pos and neg sample.
        if multi_task:
            masked_lm_loss = loss_mask_and_normalize_classify(
                masked_lm_loss.float(), masked_weights, next_sentence_label)  # 在这里加input sample mask
        else:
        ...
        # 不同任务的loss求和
        all_lm_loss = masked_lm_loss + masked_bert_loss/perf_label.size(0)
        # all_lm_loss = masked_bert_loss
        # all_lm_loss = masked_cnn_loss
        if step % 3 == 0:
            print('masked_lm_loss:{0}, masked_bert_loss:{1}, all_lm_loss:{2}'.format(
                masked_lm_loss, masked_bert_loss/perf_label.size(0), all_lm_loss
            ))
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
                 ...
                 not_predict_set=None, ngram_size=3, min_len=0, mode="s2s", pos_shift=False):
        ...
        self.apply(self.init_bert_weights)
        # self.cnnText = CNN_Text(config)
        # self.bert_classifier = NEWBertForTokenClassification(config)
        self.bert_num_labels = 3
        self.bert_classifier = NEWBertForTokenClassification.from_pretrained('bert-base-chinese', 
            num_labels = self.bert_num_labels, return_dict=True)
        # self.bert_classifier = NEWBertForTokenClassification(config)
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None, 
            stu_com_ids=None, s2s_sep_idx=None, classifier_attention_mask=None, com_cls_idx=None):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx, 
                mask_qkv=mask_qkv, s2s_sep_idx=s2s_sep_idx, classifier_attention_mask=classifier_attention_mask,
                com_cls_idx=com_cls_idx)
        ...
        while next_pos < output_length:
            ...
            # self.bert每次得到上一轮预测的token和当前待预测的mask token的变量；
            ...
            last_hidden = new_encoded_layers[-1][:, -1:, :]  # 取出最后一层最后一个词的隐藏层变量
            ...
            _, max_ids = torch.max(prediction_scores, dim=-1)  # 取出概率最大的id
    ...
    def cnn_trans_input_id2emb(self, input_ids, input_length, s2s_sep_idx, classifier_attention_mask, com_cls_idx):
        """
        将输入id经过分类器，做点评项的重排序，最终输出为id形式
        """
        # TODO：去除x_input_ids的首尾
        cls_id = torch.index_select(input_ids, 1, torch.tensor([0]).cuda())
        sep_id = torch.index_select(input_ids, 1, torch.tensor([input_length-1]).cuda())
        bert_input_ids = torch.index_select(input_ids, 1, torch.tensor(range(1, input_length-1)).cuda())
        cnn_pred_labels = []
        if input_ids is not None:
            outputs = self.bert_classifier(input_ids=bert_input_ids, attention_mask=classifier_attention_mask)
            token_logits = outputs.logits
            for i, (input_id, logit, sep_idx, cls_idx) in enumerate(zip(input_ids, token_logits, s2s_sep_idx, com_cls_idx)):
                # word_com_ids = stu_com_id.view(-1, 10)
                # logit = self.cnnText(word_com_ids)  # 分类器 
                COM_SEP_END = torch.index_select(input_id, 0, torch.tensor(range(sep_idx[0], len(input_id))).cuda()).view(1, -1)
                stu_com_id = torch.index_select(input_id, 0, torch.tensor(range(1, sep_idx[0])).cuda())
                word_com_ids = stu_com_id.view(-1, 10)
                cls_idx = torch.index_select(cls_idx, 0, torch.tensor(range(word_com_ids.size(0))).cuda())
                ...
                cnn_pred_labels.append(torch.argmax(logit, dim=1).tolist())
                gumbel_logit = self.cls2gen.gumbel_softmax(logit)
                one_cnn_result = self.cls2gen.comment_choice(word_com_ids.float(), gumbel_logit)  # 实现点评项的重排序
                one_cnn_result = one_cnn_result.view(1, -1)
                one_cnn_result = torch.cat([one_cnn_result.long(), COM_SEP_END], dim=1)
                if i == 0:
                    cnn_result = one_cnn_result
                else:
                    cnn_result = torch.cat([cnn_result, one_cnn_result], dim=0)
            new_input_ids = torch.cat([cls_id, cnn_result.long()], dim=1)
        return new_input_ids, cnn_pred_labels

    def concat_input_comment(self, x_input_ids, input_length, cnn_result):
        comment_ids = torch.index_select(x_input_ids, 1, torch.tensor(range(input_length-1, x_input_ids.size(1))).cuda())
        comment_embs = self.embed(comment_ids)
        cls_emb = self.embed(torch.index_select(x_input_ids, 1, torch.tensor([0]).cuda()))
        input_embs = torch.cat([cls_emb, cnn_result, comment_embs], dim=1)
        return input_embs


    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None,
                    s2s_sep_idx=None, classifier_attention_mask=None, com_cls_idx=None):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        ...
        # 将输入点评项经过cnn模型分类，并进行重排序；
        trans_input_id, cnn_pred_label = self.cnn_trans_input_id2emb(input_ids, input_length, s2s_sep_idx, 
            classifier_attention_mask, com_cls_idx)
        curr_ids = copy.deepcopy(trans_input_id)
        ...
        while next_pos < output_length:
            ...
            # 在此处将input_ids和预测的编码转换为x_input_embs
            # if x_input_ids.size(0) == batch_size:
            #     input_embs = self.concat_input_comment(x_input_ids, input_length, cnn_result)
            #     new_embedding, new_encoded_layers, pooled_output = \
            #         self.bert(input_embs, curr_token_type_ids, curr_position_ids, curr_attention_mask,
            ...
            #     last_hidden, None, task_idx=task_idx)
            prediction_scores, seq_relationship_score = self.cls(
                last_hidden, pooled_output, task_idx=task_idx)
            ...
            if len(total_scores) == 0:
                ...
                back_ptrs = torch.zeros(batch_size, K)
                ...
                back_ptrs = torch.div(k_ids, K)  # back_ptrs存储的是上一个时刻的token的位置；
                kk_ids = torch.reshape(kk_ids, [batch_size, K * K])
                k_ids = torch.gather(kk_ids, 1, k_ids)  # k_ids存储token对应的ids；
            ...
            if self.forbid_duplicate_ngrams:
                ...
                if len(partial_seqs[0]) >= self.ngram_size:
                    ...
                    if max(len(x) for x in dup_cands) > 0:
                        ...
                        forbid_word_mask = None
            next_pos += 1
            if next_pos == output_length:
                max_prob, max_label = torch.max(F.softmax(seq_relationship_score, dim=-1), dim=-1)
                # _, max_y = torch.max(seq_relationship_score, dim=-1)
                # sample_y = [torch.multinomial(score, 1) for score in F.softmax(seq_relationship_score, dim=2)]
        ...
        traces = {'pred_seq': [], 'scores': [], 'wids': [], 'ptrs': [], 'ppl': [], 'pred_label_prob': max_prob.tolist(), 
                'pred_label': max_label.tolist(), 'trans_input_id': trans_input_id.tolist(), 'cnn_pred_label': cnn_pred_label}
        for b in range(batch_size):
            # [(beam,)]
            ...
            if frame_id == -1:
                ...
                traces['ppl'].append(10000)  # 没有输出，因此设置一个极大的困惑度; add by sfeng
            else:
                seq = [wids_list[frame_id][pos_in_frame]]
                # ppl = [scores[frame_id][pos_in_frame]]
                for fid in range(frame_id, 0, -1):  # 根据对应索引找到相应序列；
                    pos_in_frame = ptrs[fid][pos_in_frame]  # ptrs存储上一个时刻的token位置；
                    seq.append(wids_list[fid - 1][pos_in_frame])
                    # ppl.append(scores[fid - 1][pos_in_frame])
                seq.reverse()
                traces['pred_seq'].append(seq)
                # traces['ppl'].append(-sum(ppl)/(frame_id+1))   # 添加困惑度指标; add by sfeng
                traces['ppl'].append(-max_score/(frame_id+1))   # 添加困惑度指标; add by sfeng
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, mask_qkv=None, task_idx=None):
        ...
        if labels is not None:
            ...
            # return logits
            max_prob, max_label = torch.max(F.softmax(logits, dim=-1), dim=-1)
            return max_prob.tolist(), max_label.tolist()
