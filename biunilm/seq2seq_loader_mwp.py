...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, memory_file, batch_size, tokenizer, 
            bi_uni_pipeline=[], topk=2, args=None, question_sim_ids_path=''):
        super().__init__()
        ...
        self.index_ids = {}
        self.id_questions = {}
        self.question_sim_ids = []
        self.args = args

        question_sim_ids_dict = pickle.load(open(question_sim_ids_path, 'rb'))
        ids_questions_dict = pickle.load(open(self.args.ids_questions_path, 'rb'))
        ...
        for idx, (id, value) in enumerate(question_sim_qsas.items()):
            ...
            self.question_sim_ids.append([id] + question_sim_ids_dict[id])

            if id in self.id_questions:
                sub_ex_list = self.id_questions[id]
            else:
                sub_ex_list = self.tokenize_sample(primary_question)
                self.id_questions[id] = copy.deepcopy(sub_ex_list)
            sample.append(sub_ex_list)

            sim_ids = question_sim_ids_dict[id]
            for sim_idx, sim_id in enumerate(sim_ids):
                if sim_idx >= self.args.retrieve_topn:
                    break
                if sim_id in self.id_questions:
                    sample.append(self.id_questions[sim_id])
                else:
                    sim_question = [ids_questions_dict[sim_id]['question'], ids_questions_dict[sim_id]['equation']]
                    sub_ex_list = self.tokenize_sample(sim_question)
                    self.id_questions[sim_id] = copy.deepcopy(sub_ex_list)
                    sample.append(sub_ex_list)
            self.ex_list.append(sample)
    ...
    def tokenize_sample(self, primary_question):
        pri_src, pri_tgt = self.tokenize_input(primary_question[0], primary_question[1])
        # add for keeping unk words used by copy2;
    ...
    def __getitem__(self, idx):
        ...
        qs_sim_ids = self.question_sim_ids[idx]
        new_instance = []
        for id, question in zip(qs_sim_ids, instances):
            pro_instance = self.bi_uni_pipeline(question)
            if len(pro_instance) > 1:
                new_instance.append([id] + pro_instance)
            else:
                new_instance.append([id, pro_instance])
        return new_instance
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        ret_tokens_a_wo_padding, ret_tokens_b = ret_tokens_a_wo_padding[0], ret_tokens_b[0]
        ...
        position_ids = position_ids[:self.max_analogy_len]
        ...
        num_equ_ids = num_equ_ids[:self.max_analogy_len]
...
class Preprocess4Seq2seqBaseline(Pipeline):
    ...
    def __call__(self, instance):
        query_question, query_equation = instance  # ret=retrieve
        query_tokens_a_wo_padding, query_tokens_a_wo_padding_keep_unk = query_question
        query_tokens_b, query_tokens_b_keep_unk = query_equation

        tokens_a_wo_padding = query_tokens_a_wo_padding[:self.max_len_a - 2]
        tokens_a = ['[CLS]'] + tokens_a_wo_padding + ['[SEP]']
        if len(tokens_a) < self.max_len_a:
            tokens_a += ['[PAD]'] * (self.max_len_a - len(tokens_a))  
        else:
            tokens_a = tokens_a[:self.max_len_a]
        # Token Indexing
        ...
        # add segment_ids
        segment_ids = [0]*self.max_len_a 

        # add position_ids, when add tokens_a_padding, 原来模型中的position_ids没有跳过padding部分，所以此处做修改；
        position_ids = [i for i in range(len(query_tokens_a_wo_padding) + 2)]
        position_ids += [0] * (self.max_len_a - len(query_tokens_a_wo_padding) - 2)
        position_ids = position_ids[:self.max_len_a]
        ...
        # input mask.
        input_mask = torch.zeros(self.max_len_a, self.max_len_a, dtype=torch.long)
        ...
        if self.is_train:
            return [input_ids, segment_ids, input_mask, mask_qkv, query_masked_ids, query_masked_pos, query_masked_weights, ret_masked_ids, 
                    ret_masked_pos, ret_masked_weights, -1, self.task_idx, analogy_attention_mask, position_ids, num_equ_ids, 
                    extend_oov_query_qsids, oov_tokens]
        else:
            return [input_ids, segment_ids, input_mask, mask_qkv, self.task_idx, analogy_attention_mask, position_ids, num_equ_ids,
                    extend_oov_query_qsids, oov_tokens]
