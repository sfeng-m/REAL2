...
def raw_batch_list_to_batch_tensors(batch, is_train=True):
    batch_tensors = []
...
'''
def pad_label_ids(batch):
    """动态类别过程中，每一个样本的类别数不一样，因此此处进行padding，并提取出样本类别id与总表id的映射关系"""
    vocabId_batchId = {}
    batchId_vocabId = {}
    index = 0
    id_len = []
    for bt in batch:
        ...
        for label_id in label_ids:
            if label_id in vocabId_batchId:
                batch_ids.append(vocabId_batchId[label_id])
            elif label_id != 0:
                index += 1
                vocabId_batchId[label_id] = index
                batch_ids.append(index)
        bt[7] = batch_ids
        id_len.append(len(batch_ids))
    max_len = max(id_len)
    for bt in batch:
        if bt[7] < max_len:
            bt[7].extend([0]*(max_len-len(bt[7])))
    for vocabId, batchId in vocabId_batchId.items():
        batchId_vocabId[batchId] = vocabId
    return batch, batchId_vocabId
'''
