...
from pytorch_pretrained_bert.modeling_mwp import BertForPreTrainingLossMask, BertForSeq2SeqDecoder
...
def main(args, logger):
    ...
    if args.do_train:
        ...
        bi_uni_pipeline.append(seq2seq_loader.Preprocess4Seq2seq(args.max_pred, list(tokenizer.vocab.keys(
                ...
                max_analogy_len=args.max_analogy_len, is_train=True, copynet_name=args.copynet_name))

        train_dataset = seq2seq_loader.Seq2SeqDataset(
            args.memory_train_file, args.train_batch_size, data_tokenizer,  
            bi_uni_pipeline=bi_uni_pipeline[0], topk=args.topk, args=args, 
            question_sim_ids_path=args.train_question_sim_ids_path)
    
        id_questions = train_dataset.id_questions
        question_sim_ids = train_dataset.question_sim_ids
        print('finish id_instances')
        id_instances = None
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=_batch_size, sampler=train_sampler,
                                                       num_workers=args.num_workers, collate_fn=seq2seq_loader.raw_batch_list_to_batch_tensors, pin_memory=False, drop_last=True) 
    ...
    args.model_recover_path = None  # add by sfeng
    ...
    model = BertForPreTrainingLossMask.from_pretrained(
        ...
        add_analogy_embedding=args.add_analogy_embedding, vocabs=vocabs, topk=args.topk,
        args=args
        )
    ...
    # print('param_optimizer:%s' % model.named_parameters()['loss_beta'])
    ...
    if recover_step:
        ...
        # optim_recover = torch.load(os.path.join(
        #     args.output_dir, "optim_lrDecay.{0}.bin".format(recover_step)), map_location='cpu')
        if hasattr(optim_recover, 'state_dict'):
            optim_recover = optim_recover.state_dict()
        # print('optim_recover:%s' % optim_recover)
    ...
    if args.do_train:
        ...
        # lr_decay_speed = 2
        lr_decay_speed = 8  # TODO: tmp for 35 start, must delete if start < 25
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                batch = [
                    t.to(device) if t is not None and not isinstance(t[0], str) else t for t in batch]

                question_sim_ids, input_ids, segment_ids, input_mask, mask_qkv, query_masked_ids, query_masked_pos, query_masked_weights, \
                ...
                if not args.add_num_equ_ids:
                    num_equ_ids = None
                loss = model(input_ids, segment_ids, input_mask, ret_masked_ids, masked_pos=ret_masked_pos, masked_weights=ret_masked_weights, 
                                ...
                                question_sim_ids=question_sim_ids, bi_uni_pipeline=bi_uni_pipeline[1], id_questions=id_questions, id_instances=id_instances) 
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    # if args.fp16:
                    if args.used_lr_decay and i_epoch > args.start_lr_decay_epoch:
            ...
            if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                if i_epoch == 1 or i_epoch % 5 == 0 or i_epoch >= args.num_train_epochs-10 or args.save_every_epoch:  # 每5个epoch保存一次
                    output_model_file = os.path.join(
                        args.output_dir, "model{0}.{1}.bin".format(args.repeat, i_epoch))
                    torch.save(model_to_save.state_dict(), output_model_file)
                    output_optim_file = os.path.join(
                        args.output_dir, "optim{0}.{1}.bin".format(args.repeat, i_epoch))
