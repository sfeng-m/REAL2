...
DataSet = 'math23k'  # math23k or math23k_test1k
Topk = 3
retrieve_topn = 5
is_train = False
retrieve_model_name = 'bert'  # 目前支持cnn, rcnn, bert
TRAIN_EPOCHS = 50
LR_Decay = True  # 是否进行学习率衰减
Start_LR_Decay_Epoch = 25
REPEAT = ''
BEAM_SIZE = 1

Add_Copynet = True   # copy mechanism
Add_Memory_Module = True  # memory module
is_equ_norm = True  # 是否对表达式进行等式归一化处理
add_analogy_embedding = True # 如果添加检索数据，则为True，否则为False
Bert_Model = 'bert-base-chinese'


if REPEAT == '':
    SEED = 42
elif REPEAT == '_repeat2':
    SEED = 2021
elif REPEAT == '_repeat3':
    SEED = 811
...
def get_args():
    ...
    # Required parameters
    parser.add_argument("--bert_model", default=Bert_Model, type=str, dest='bert_model',
    ...
    # parser.add_argument('--pos-shift', action='store_true', dest='pos_shift',
    ...
    parser.add_argument('--is_delete_early_model', type=bool, 
                        default=False,
                        dest='is_delete_early_model',
                        help="is delete early model")
    parser.add_argument('--add_memory_module', type=bool, 
                        default=Add_Memory_Module,
                        dest='add_memory_module',
    ...
    parser.add_argument('--add_tokens_a_padding', type=bool, default=True, dest='add_tokens_a_padding',
                        help="add question padding so that question length is same when process question and equation.")
    parser.add_argument('--add_copynet', type=bool, default=Add_Copynet, dest='add_copynet',
                        help="add copynet module")
    parser.add_argument('--add_analogy_embedding', type=bool, default=add_analogy_embedding, dest='add_analogy_embedding',
                        help="add analogy embedding")
    parser.add_argument('--save_every_epoch', type=bool, default=False, dest='save_every_epoch',
                        help="save_every_epoch")
    parser.add_argument('--pred_wo_memory_copy', type=bool, default=False, dest='pred_wo_memory_copy',
                        help="pred without memory module and copy module.")
    parser.add_argument('--easy_to_hard', type=bool, default=True, dest='easy_to_hard',
    ...
    parser.add_argument('--used_lr_decay', type=bool, default=LR_Decay, dest='used_lr_decay',
                        help="whether to use lr decay function.")
    parser.add_argument('--Fold', type=int, default=1, dest='Fold',
                        help="whether to use 5 fold cross validation and use which fold.")
    parser.add_argument('--start_lr_decay_epoch', type=int, default=Start_LR_Decay_Epoch, dest='start_lr_decay_epoch',
                        help="the epoch starting lr decay.")
    parser.add_argument('--dataset', type=str, default=DataSet, dest='dataset',
                        help="which dataset, math23k/ape210k.")
    parser.add_argument('--used_bertAdam', type=bool, default=False, dest='used_bertAdam',
                        help="optimizer use bertAdam or adam.")
    parser.add_argument('--is_equ_norm', type=bool, default=is_equ_norm, dest='is_equ_norm',
                        help="whether use equation normalization.")
    parser.add_argument('--repeat', type=str, default=REPEAT, dest='repeat',
                        help="repeat train with ape dataset.") 
    parser.add_argument('--is_train',  action='store_true', dest='is_train',
                        help="train or valid.")
    parser.add_argument('--train_question_sim_ids_path', type=str, default='', dest='train_question_sim_ids_path',
                        help="train question similar ids path.") 
    parser.add_argument('--valid_question_sim_ids_path', type=str, default='', dest='valid_question_sim_ids_path',
                        help="valid question similar ids path.") 
    parser.add_argument('--ids_questions_path', type=str, default='', dest='ids_questions_path',
                        help="id-question dict path.")
    parser.add_argument('--copynet_name', type=str, default='copynet2', dest='copynet_name',
                        help="decide which copynet way to use.") 
    parser.add_argument('--retrieve_model_name', type=str, default=retrieve_model_name, dest='retrieve_model_name',
                        help="decide which retrieve model to use.") 
    parser.add_argument('--retrieve_topn', type=int, default=retrieve_topn, dest='retrieve_topn',
                        help="输入检索模块之前的topn个相似样本（由无监督相似度计算得到）.")
    parser.add_argument('--retrieve_result_path', type=str, default='', dest='retrieve_result_path',
                        help="经过检索器后的检索结果保存地址，方便检索器的定量及定性分析.") 
    ...
    parser.add_argument('--batch_size', type=int, default=PRED_BATCH_SIZE, dest='batch_size',
                    help="Batch size for decoding.")
    parser.add_argument('--beam_size', type=int, default=BEAM_SIZE, dest='beam_size',
